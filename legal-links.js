(() => {
  const render = () => {
    ...
    document.querySelectorAll('.v2-footer, .syn-v2-footer, footer').forEach(footer => {
      ...
      wrap.style.cssText = ['display:flex','gap:18px','flex-wrap:wrap','align-items:center','font-size:11px','margin-top:10px','opacity:.82'].join(';');
      const links = [
  ...
  window.addEventListener('storage', event => { if (event.key === 'phoenix-lang') render(); });
