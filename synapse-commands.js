(() => {
  ...
  function applyStatic(){const t=text[lang];document.documentElement.lang=lang;document.title=lang==='fr'?'Commandes Synapse — Phoenix Inc | Development':'Synapse Commands — Phoenix Inc | Development';document.querySelectorAll('[data-cmd-nav]').forEach(el=>{el.textContent=t.nav[el.dataset.cmdNav]||el.textContent;});document.querySelectorAll('[data-cmd-text]').forEach(el=>{const key=el.dataset.cmdText;if(t[key]!=null)el.textContent=t[key];});document.querySelectorAll('[data-cmd-html]').forEach(el=>{const key=el.dataset.cmdHtml;if(t[key]!=null)el.innerHTML=t[key];});if(search)search.placeholder=t.placeholder;toggle?.classList.toggle('en',lang==='en');}
