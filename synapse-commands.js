(() => {
  const data = [
    {cat:'builder',fr:'/analyse_serveur',en:'/analyze_server',frDesc:'Analyse la structure et la DA du serveur sans rien modifier.',enDesc:'Analyzes the server structure and visual style without changing anything.',access:'managers'},
    ...
    {cat:'builder',fr:'/builder ajouter_salon',en:'/builder add_channel',frDesc:'Ajoute un salon en reprenant la DA de la catégorie choisie.',enDesc:'Adds a channel matching the visual style of the selected category.',access:'managers'},
    {cat:'builder',fr:'/builder ajouter_categorie',en:'/builder add_category',frDesc:'Ajoute une catégorie dans la DA détectée du serveur.',enDesc:'Adds a category matching the detected server style.',access:'managers'},
    ...
    {cat:'builder',fr:'/reinitialiser_salons',en:'/reset_channels',frDesc:'Sauvegarde puis supprime tous les salons et catégories après confirmation. Les rôles restent.',enDesc:'Backs up then deletes every channel and category after confirmation. Roles are kept.',access:'discordAdmin'},
    ...
    {cat:'tickets',fr:'/tickets type_ajouter',en:'/tickets add_type',frDesc:'Ajoute un type de ticket personnalisé et actualise le panel.',enDesc:'Adds a custom ticket type and refreshes the panel.',access:'managers'},
    {cat:'tickets',fr:'/tickets type_retirer',en:'/tickets remove_type',frDesc:'Retire un type personnalisé et actualise le panel.',enDesc:'Removes a custom ticket type and refreshes the panel.',access:'managers'},
    ...
    {cat:'community',fr:'/annonce',en:'/announcement',frDesc:'Publie une annonce structurée via un formulaire.',enDesc:'Publishes a structured announcement through a form.',access:'staff'},
    {cat:'community',fr:'/recherche',en:'/search',frDesc:'Publie une recherche ou une demande structurée. Accessible à tous.',enDesc:'Publishes a structured search or request. Available to everyone.',access:'everyone'},
    ...
    {cat:'community',fr:'/commandes',en:'/commands',frDesc:'Affiche la liste des commandes directement dans Discord.',enDesc:'Shows the command list directly inside Discord.',access:'everyone'},
    {cat:'community',fr:'/aide_synapse',en:'/synapse_help',frDesc:'Affiche une aide rapide.',enDesc:'Shows quick Synapse help.',access:'everyone'},
    ...
    {cat:'access',fr:'/permissions autoriser_role',en:'/permissions allow_role',frDesc:'Autorise un rôle précis pour une commande.',enDesc:'Explicitly allows a role for a command.',access:'managers'},
    ...
    {cat:'interserver',fr:'/lier_avec_code',en:'/link_with_code',frDesc:'Relie un salon à un autre serveur via un code.',enDesc:'Links a channel to another server using a code.',access:'discordAdmin'},
  ...
  const text = {
    fr:{
      nav:{home:'Accueil',scripts:'Scripts',bots:'Bots',commands:'Commandes',services:'Web · Apps · Autres'},
      kicker:'SYNAPSE / COMMANDES',title:'Tout Synapse,<br><span>directement dans Discord.</span>',lead:'Retrouvez les commandes disponibles pour construire, adapter et administrer Synapse directement depuis Discord.',helpCommand:'/commandes',helpText:'Affiche toutes les commandes directement dans Discord.',back:'Voir la fiche Synapse →',placeholder:'Rechercher une commande…',all:'Toutes',copy:'Copier',copied:'Copié ✓',empty:'Aucune commande ne correspond à votre recherche.',futureKicker:'MISE À JOUR FUTURE',futureTitle:'Dashboard Web Synapse',futureCopy:'Un Dashboard Web complet est en développement pour les futures versions de Synapse. Il viendra compléter le pilotage depuis Discord avec une interface visuelle centralisée.',futureBuilder:'Structure & adaptation',security:'SÉCURITÉ',futureSecurity:'AutoMod & protections',multi:'MULTI-SERVEURS',futureMulti:'Gestion centralisée',
      cats:{builder:'Builder & adaptation',tickets:'Tickets',community:'Communauté',access:'Accès & permissions',premium:'Premium',interserver:'Interserver'},
      access:{everyone:'Tous',staff:'Staff',managers:'Responsables',discordAdmin:'Admin Discord'}
    },
    en:{
      nav:{home:'Home',scripts:'Scripts',bots:'Bots',commands:'Commands',services:'Web · Apps · More'},
      kicker:'SYNAPSE / COMMANDS',title:'All of Synapse,<br><span>directly inside Discord.</span>',lead:'Browse the commands available to build, adapt and manage Synapse directly from Discord.',helpCommand:'/commands',helpText:'Shows every command directly inside Discord.',back:'View Synapse →',placeholder:'Search a command…',all:'All',copy:'Copy',copied:'Copied ✓',empty:'No command matches your search.',futureKicker:'FUTURE UPDATE',futureTitle:'Synapse Web Dashboard',futureCopy:'A complete Web Dashboard is being developed for future Synapse versions. It will complement Discord controls with a centralized visual interface.',futureBuilder:'Structure & adaptation',security:'SECURITY',futureSecurity:'AutoMod & protection',multi:'MULTI-SERVER',futureMulti:'Centralized management',
      cats:{builder:'Builder & adaptation',tickets:'Tickets',community:'Community',access:'Access & permissions',premium:'Premium',interserver:'Interserver'},
      access:{everyone:'Everyone',staff:'Staff',managers:'Managers',discordAdmin:'Discord Admin'}
    }
  };
  ...
  function applyStatic(){
    ...
    document.title=lang==='fr'?'Commandes Synapse — Phoenix Inc | Development':'Synapse Commands — Phoenix Inc | Development';
    document.querySelectorAll('[data-cmd-nav]').forEach(el=>{el.textContent=t.nav[el.dataset.cmdNav]||el.textContent;});
    document.querySelectorAll('[data-cmd-text]').forEach(el=>{const key=el.dataset.cmdText;if(t[key]!=null)el.textContent=t[key];});
    document.querySelectorAll('[data-cmd-html]').forEach(el=>{const key=el.dataset.cmdHtml;if(t[key]!=null)el.innerHTML=t[key];});
    if(search)search.placeholder=t.placeholder;
  ...
  toggle?.addEventListener('click',()=>{lang=lang==='fr'?'en':'fr';localStorage.setItem('phoenix-lang',lang);refresh();});
